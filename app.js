...
function renderHome() {
  ...
  screens.home.innerHTML = `
    <button class="card home-action-card" type="button" data-home-action="timetable">
      ...
      <div class="home-action-hint">눌러서 시간표 자세히 보기</div>
    </button>

    <button class="card home-action-card" type="button" data-home-action="meal">
      ...
      <div class="home-action-hint">눌러서 급식 자세히 보기</div>
    </button>
...
function bindHomeEvents() {
  screens.home.querySelectorAll("[data-home-action='timetable']").forEach((button) => {
    button.addEventListener("click", () => {
      switchTab("timetable");
    });
  });

  screens.home.querySelectorAll("[data-home-action='meal']").forEach((button) => {
    button.addEventListener("click", () => {
      switchTab("meal");
    });
  });
...
function renderProfile() {
  ...
  const teacherOnlyFields = `
      <label class="profile-key" for="teacherSubjectInput">담당 과목</label>
      <input id="teacherSubjectInput" class="comment-input" maxlength="30" value="${escapeHtml(profile.teacherSubject || "")}" placeholder="예: 수학" />
      <p class="lesson-meta">선생님 모드에서는 선생님 관리 항목만 표시돼요.</p>
  `;

  screens.profile.innerHTML = `
    <h2 class="panel-title">내 정보</h2>
    <p class="lesson-meta">현재 시작 유형: ${isTeacherMode ? "선생님" : "학생"} (새로고침하면 다시 선택할 수 있어요)</p>
    <form id="profileForm" class="post-form">
      ...
      ${isTeacherMode ? teacherOnlyFields : studentOnlyFields}
  ...
  if (profileForm) {
    profileForm.addEventListener("submit", (event) => {
      ...
      state.data.profile.name = normalizeText(screens.profile.querySelector("#profileNameInput")?.value || "", 40);

      if (isTeacherMode) {
        state.data.profile.teacherSubject = normalizeText(
          screens.profile.querySelector("#teacherSubjectInput")?.value || "",
          30
        );
      } else {
        ...
        const schoolLevel = normalizeText(
          String(screens.profile.querySelector("#schoolLevelInput")?.value || "mis").toLowerCase(),
          10
        );
        const grade = normalizeText(screens.profile.querySelector("#gradeInput")?.value || "", 2);
        const classNo = normalizeText(screens.profile.querySelector("#classNoInput")?.value || "", 2);

        if (rawOfficeCode && !cleanedOfficeCode) {
          codeAdjusted = true;
        }

        if (rawSchoolCode && !cleanedSchoolCode) {
          codeAdjusted = true;
        }
        ...
        if (classNo && !/^\d{1,2}$/.test(classNo)) {
          showToast("반은 숫자로 입력해 주세요");
          return;
        }

        state.data.profile.officeCode = cleanedOfficeCode;
        state.data.profile.schoolCode = cleanedSchoolCode;
        state.data.profile.schoolLevel = ["els", "mis", "his"].includes(schoolLevel)
          ? schoolLevel
          : "mis";
        state.data.profile.grade = grade || "1";
        state.data.profile.classNo = classNo || "1";
      }
...
function openStartupRolePicker() {
  const existing = document.querySelector("#startupRolePicker");
  if (existing) existing.remove();

  const overlay = document.createElement("div");
  overlay.id = "startupRolePicker";
  overlay.className = "startup-role-picker";
  overlay.innerHTML = `
    <div class="startup-role-card" role="dialog" aria-modal="true" aria-label="시작 사용자 유형 선택">
      <h2>시작할 때 사용자 유형 선택</h2>
      <p>이번 실행에서 학생/선생님 화면을 고르세요.</p>
      <div class="startup-role-actions">
        <button type="button" class="startup-role-btn" data-role="student">학생으로 시작</button>
        <button type="button" class="startup-role-btn" data-role="teacher">선생님으로 시작</button>
      </div>
    </div>
  `;

  overlay.querySelectorAll(".startup-role-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const role = btn.getAttribute("data-role") === "teacher" ? "teacher" : "student";
      state.data.profile.role = role;
      if (role !== "teacher") {
        clearTeacherSession();
      }
      persistData();
      ...
      showToast(`${role === "teacher" ? "선생님" : "학생"} 모드로 시작했어요`);
    });
  });
